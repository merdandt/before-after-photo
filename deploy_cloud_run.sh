...
if [ -f .env.local ]; then
    ...
    # Read PROJECT_ID from .env.local (|| [ -n "$key" ] handles files without trailing newline)
...
# Get git commit SHA or use timestamp
if git rev-parse HEAD >/dev/null 2>&1; then
    COMMIT_SHA=$(git rev-parse --short HEAD)
    echo -e "${GREEN}Using git commit: ${COMMIT_SHA}${NC}"
else
    COMMIT_SHA=$(date +%Y%m%d-%H%M%S)
    echo -e "${YELLOW}Not a git repo, using timestamp: ${COMMIT_SHA}${NC}"
fi
